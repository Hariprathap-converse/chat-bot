...
import { FileJson, ClipboardList, User, MapPin, Receipt, Calendar, CreditCard, Box } from "lucide-react";
...
export function JsonMessage({ data, title }: JsonMessageProps) {
    ...
    const getIcon = (key: string) => {
        ...
        if (k.includes('total') || k.includes('amount') || k.includes('subtotal') || k.includes('tax')) return <CreditCard className="w-4 h-4" />;
                ...
                <div className="p-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {mainDetails.map(([key, value]) => (
                            <div key={key} className="flex flex-col gap-1.5 p-3 rounded-xl bg-slate-50/50 dark:bg-muted/30 border border-slate-100/50 dark:border-border/50 group hover:border-indigo-200 dark:hover:border-indigo-900/50 transition-colors">
                    ...
                    {items.length > 0 && (
                        <div className="mt-6">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="text-xs font-bold text-slate-400 dark:text-muted-foreground uppercase tracking-widest">Line Items</h3>
                            </div>
                                    ...
                                    <table className="w-full text-left border-collapse min-w-[500px]">
                                        <thead>
                                            <tr className="bg-slate-50/50 dark:bg-muted/30">
                                                <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 dark:text-muted-foreground uppercase tracking-wider border-b border-slate-100 dark:border-border">Description</th>
                                                <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 dark:text-muted-foreground uppercase tracking-wider border-b border-slate-100 dark:border-border text-center">Qty</th>
                                                <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 dark:text-muted-foreground uppercase tracking-wider border-b border-slate-100 dark:border-border text-right">Unit Price</th>
                                                <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 dark:text-muted-foreground uppercase tracking-wider border-b border-slate-100 dark:border-border text-right">Amount</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-50 dark:divide-border/50">
                                            {items.map((item: any, idx: number) => (
                                                <tr key={idx} className="hover:bg-slate-50/30 dark:hover:bg-accent/5 transition-colors group">
                                                    <td className="px-4 py-3 text-xs font-semibold text-slate-700 dark:text-foreground group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                                        {item["Items(Description)"] || item.Description || "N/A"}
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-3 text-xs font-semibold text-slate-600 dark:text-muted-foreground text-right tabular-nums">
                                                        {item["Items(UnitPrice)"] || item.UnitPrice || "0.00"}
                                                    </td>
