          ...
          <table className="w-full text-left border-collapse min-w-full">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-border">
              {paginatedItems.map((item, idx) => (
                <tr
                  key={idx}
                  className="hover:bg-blue-50/10 dark:hover:bg-accent/5 transition-colors group cursor-pointer"
                >
                  {headers.map((header) => (
                    <td
                      key={header}
                      className="px-5 py-4 text-sm min-w-[160px]  font-semibold text-slate-700 dark:text-foreground group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"
                    >
