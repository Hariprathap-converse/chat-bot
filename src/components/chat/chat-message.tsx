...
import { Copy, CopyCheck, Edit, FileArchive, FileDiff, HardDriveDownload, MoreHorizontal, ThumbsDown, ThumbsUp } from "lucide-react";
...
export function ChatMessage({ message, index, isUser }: ChatMessageProps) {
    ...
    const [reaction, setReaction] = useState<{ liked: boolean; disliked: boolean }>({ liked: false, disliked: false });
        ...
        return (
            <div className="flex w-full justify-end">
                <div className="flex group relative flex-col items-end justify-end gap-[4px]">
                    {/* User Actions */}
                    <div className={cn("opacity-0 group-hover:opacity-100 flex gap-3 items-center h-[10px]", isOpen ? "opacity-100" : "opacity-0")}>
                        {copied ? (
                            <CopyCheck className="h-[14px] w-[14px] hover:text-accent-foreground cursor-pointer text-green-500" />
                        ) : (
                            <Copy className="h-[14px] w-[14px] hover:text-accent-foreground cursor-pointer text-sub-title" onClick={handleCopy} />
                        )}
                        ...
                        <Popover onOpenChange={setIsOpen}>
                            ...
                            <PopoverContent className="z-50 p-[6px] w-40 bg-white rounded-md shadow-md">
                                <div className="flex flex-col gap-1">
                                    <Button variant="ghost" className="flex group items-center gap-2 p-1 py-1 cursor-pointer focus:ring-0 focus-visible:ring-0 focus:ring-offset-0 focus-visible:ring-offset-0 justify-start h-fit text-sm text-foreground rounded">
                                        <FileArchive className="h-[14px] w-[14px] text-muted-foreground group-hover:text-accent-foreground" /> Option 1
                                    </Button>
                                    <Button variant="ghost" className="flex group items-center gap-2 p-1 py-1 cursor-pointer focus:ring-0 focus-visible:ring-0 focus:ring-offset-0 focus-visible:ring-offset-0 justify-start h-fit text-sm text-foreground rounded">
                                        <FileDiff className="h-[14px] w-[14px] text-muted-foreground group-hover:text-accent-foreground" /> Option 2
                                    </Button>
                                    <Button variant="ghost" className="flex group items-center gap-2 p-1 py-1 cursor-pointer focus:ring-0 focus-visible:ring-0 focus:ring-offset-0 focus-visible:ring-offset-0 justify-start h-fit text-sm text-foreground rounded">
                                        <HardDriveDownload className="h-[14px] w-[14px] text-muted-foreground group-hover:text-accent-foreground" /> Option 3
                                    </Button>
                                </div>
                            </PopoverContent>
                        </Popover>
                    ...
                    <div className="flex items-center w-full gap-1">
                        ...
                        <div className="min-w-8 min-h-8 ml-1 rounded-full bg-muted shadow items-center">
                            <Image src="/profile.jpg" width={40} height={40} alt="user" className="object-cover rounded-full" />
                        </div>
                    </div>
                </div>
            </div>
