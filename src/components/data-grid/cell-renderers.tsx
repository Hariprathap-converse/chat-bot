...
export const renderBadgeCell = (value: any, config?: CellTypeConfig) => {
    ...
    const variantClasses = {
        default: "bg-slate-50 text-slate-700 border-slate-200 dark:bg-slate-950 dark:text-slate-400",
        ...
        error: "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950 dark:text-rose-400",
        info: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950 dark:text-blue-400",
    };

    return (
        <Badge variant="outline" className={cn("text-xs px-2 py-0.5", variantClasses[variant])}>
            {stringValue}
        </Badge>
    );
};
...
export const renderStatusCell = (value: any, config?: CellTypeConfig) => {
    ...
    const defaultStatusColors: Record<string, { bg: string; text: string; icon: string }> = {
        active: { bg: 'bg-emerald-50 dark:bg-emerald-950', text: 'text-emerald-700 dark:text-emerald-400', icon: '●' },
        inactive: { bg: 'bg-slate-50 dark:bg-slate-950', text: 'text-slate-700 dark:text-slate-400', icon: '●' },
        pending: { bg: 'bg-amber-50 dark:bg-amber-950', text: 'text-amber-700 dark:text-amber-400', icon: '◐' },
        completed: { bg: 'bg-blue-50 dark:bg-blue-950', text: 'text-blue-700 dark:text-blue-400', icon: '✓' },
        failed: { bg: 'bg-rose-50 dark:bg-rose-950', text: 'text-rose-700 dark:text-rose-400', icon: '✗' },
    };
    ...
    return (
        <div className={cn("inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium", status.bg, status.text)}>
...
export const renderCell = (value: any, cellType?: string, config?: CellTypeConfig) => {
    switch (cellType) {
        case 'badge':
