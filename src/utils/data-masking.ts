export class DataMasking {
  private static patterns = {
    ssn: {
      mask: "XXX-XX-XXXX",
      regex: /^(\d{3})-?(\d{2})-?(\d{4})$/,
      format: (value: string) => value.replace(/(\d{3})(\d{2})(\d{4})/, "$1-$2-$3"),
    },
    "credit-card": {
      mask: "XXXX XXXX XXXX XXXX",
      regex: /^(\d{4})\s?(\d{4})\s?(\d{4})\s?(\d{4})$/,
      format: (value: string) => value.replace(/(\d{4})(\d{4})(\d{4})(\d{4})/, "$1 $2 $3 $4"),
    },
    phone: {
      mask: "(XXX) XXX-XXXX",
      regex: /^(\d{3})(\d{3})(\d{4})$/,
    ...
    license: {
      ...
      format: (value: string) => value.replace(/([A-Z0-9]{3})([A-Z0-9]{3})([A-Z0-9]{4})/, "$1-$2-$3"),
    },
    passport: {
      mask: "XXXXXXXXX",
      regex: /^([A-Z0-9]{9})$/,
  ...
  static maskValue(value: string, config: any, scenario: "display" | "edit" | "export" | "print" = "display"): string {
    ...
    const { pattern, maskChar = "X", showFirst = 0, showLast = 0, customPattern, preserveFormat } = config

    if (pattern === "custom" && customPattern) {
      return this.applyCustomMask(value, customPattern, maskChar, showLast, showFirst)
    }
    ...
    if (pattern === "email") {
      ...
      if (emailMatch) {
        ...
        const maskedUsername = this.maskPortion(username, maskChar, showFirst, showLast)
        const maskedDomain = this.maskPortion(domain, maskChar, 0, Math.min(2, domain.length))
  ...
  private static maskPortion(value: string, maskChar: string, showFirst: number, showLast: number): string {
  ...
  static applyCustomMask(value: string, pattern: string, maskChar: string, showLast: number, showFirst = 0): string {
    let masked = ""
    let valueIndex = 0

    for (let i = 0; i < pattern.length && valueIndex < value.length; i++) {
      if (pattern[i] === "X") {
        if ((showFirst > 0 && valueIndex < showFirst) || (showLast > 0 && valueIndex >= value.length - showLast)) {
