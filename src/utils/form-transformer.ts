...
export function transformBackendFormToDefinition(
  backendData: any,
): FormDefinition {
  ...
  const mappedFields: FieldConfig[] = (fields || []).map((field: any) => ({
    ...
    dataSource:
      field.dataSource ||
      (field.type === "select" && field.options ? "static" : field.dataSource),
  ...
  return {
    form: {
      ...
      formType: "basic",
      breadcrumbs: [{ label: formattedHeader }],
      mode: "create",
      layout: {
      ...
      formHeader: {
        ...
        buttonText:
          intent?.includes("create") || intent?.includes("add")
            ? "Create"
            : "Submit",
      },
      viewMode: false,
