...
export class CrossFieldValidator {
  static validate(fieldValue: any, validation: CrossFieldValidation, formData: Record<string, any>): string | null {
    const {
      dependsOn,
      rule,
      value,
    ...
    if (dependentValues.some((val) => val === undefined || val === null || val === "")) {
    ...
    switch (rule) {
      case "equals":
        ...
        const compareStr = caseSensitive ? String(compareValue) : String(compareValue).toLowerCase()
      ...
      case "not_equals":
        const notCompareValue = value !== undefined ? value : dependentValues[0]
        const notFieldStr = caseSensitive ? String(fieldValue) : String(fieldValue).toLowerCase()
        const notCompareStr = caseSensitive ? String(notCompareValue) : String(notCompareValue).toLowerCase()
      ...
      case "greater_equal":
        ...
        if (Number(fieldValue) < Number(geValue)) {
          return errorMessage || `Must be greater than or equal to ${dependsOn[0]}`
        }
        break
      ...
      case "less_equal":
        ...
        if (Number(fieldValue) > Number(leValue)) {
          return errorMessage || `Must be less than or equal to ${dependsOn[0]}`
        }
        break

      case "contains":
        const containsValue = value !== undefined ? value : dependentValues[0]
        const containsFieldStr = caseSensitive ? String(fieldValue) : String(fieldValue).toLowerCase()
        const containsSearchStr = caseSensitive ? String(containsValue) : String(containsValue).toLowerCase()
      ...
      case "not_contains":
        const notContainsValue = value !== undefined ? value : dependentValues[0]
        const notContainsFieldStr = caseSensitive ? String(fieldValue) : String(fieldValue).toLowerCase()
        const notContainsSearchStr = caseSensitive ? String(notContainsValue) : String(notContainsValue).toLowerCase()
      ...
      case "starts_with":
        ...
        const startsFieldStr = caseSensitive ? String(fieldValue) : String(fieldValue).toLowerCase()
        const startsSearchStr = caseSensitive ? String(startsValue) : String(startsValue).toLowerCase()
      ...
      case "ends_with":
        ...
        const endsFieldStr = caseSensitive ? String(fieldValue) : String(fieldValue).toLowerCase()
        const endsSearchStr = caseSensitive ? String(endsValue) : String(endsValue).toLowerCase()
      ...
      case "different_day":
        ...
        if (diffDayDate.toDateString() === diffDayFieldDate.toDateString()) {
          return errorMessage || `Must be on a different day than ${dependsOn[0]}`
        }
        break
      ...
      case "min_difference":
        ...
        if (minDifference && minActualDiff < minDifference) {
          return errorMessage || `Must have at least ${minDifference} difference from ${dependsOn[0]}`
        }
        break
      ...
      case "max_difference":
        ...
        if (maxDifference && maxActualDiff > maxDifference) {
          return errorMessage || `Must have at most ${maxDifference} difference from ${dependsOn[0]}`
        }
        break
